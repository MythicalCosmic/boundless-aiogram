...
from boundless_aiogram.core import print_error, print_info, print_colored
...
def cmd_run():
    ...
    if not project_root:
        print_error("Not inside a Boundless project (no .boundless.yml or main.py found).")
        sys.exit(1)

    config = load_config(project_root)
    mode = config.get("server_mode", "polling") if config else "polling"

    if mode == "webhook" or (mode == "both" and "--webhook" in sys.argv):
        print_info("Starting bot in webhook mode (uvicorn)...")
        print_colored(
            f"    Mode: {Colors.GREEN}Webhook (FastAPI){Colors.ENDC}",
            Colors.DIM,
        )
        subprocess.run(
            [
        ...
        if mode == "both":
            print_colored(
                f"    Tip: use {Colors.CYAN}boundless run --webhook{Colors.ENDC} for webhook mode",
                Colors.DIM,
            )
